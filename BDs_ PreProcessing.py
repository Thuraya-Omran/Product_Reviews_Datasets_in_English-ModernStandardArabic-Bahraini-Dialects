...
def getStopWordList(stopWordListFileName):
    ...
    fp = open('dialectSWD.txt', 'r',encoding='utf-8')
    line = fp.readline()
    while line:
        word = line.strip()
    ...
    fp.close()
    return myStopWords
dialect_stopwords = getStopWordList('dialectSWD.txt')
...
labels=[]
#-------------------------------
df=pd.read_csv('DC-5000-Bal.txt',sep='\t',names=['polarity','Dcomment'],encoding='utf-8')
...
for index, row in df.iterrows():
    ...
    ######################*************** preprocessing of original Dialects comments comments *************####################### 
    if __name__ == '__main__':
        args = parser.parse_args()
        ###text = args.infile.read()
...
mydataframe.to_csv('AUGMENT of 5000 BD.txt', sep='\t',header=None)
...
df=pd.read_csv('AUGMENT of 5000 BD.txt',sep='\t',names=['polarity','Dcomment'],encoding='utf-8')
neg, pos = np.bincount(df['polarity'])
total = neg + pos
